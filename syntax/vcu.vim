" Vim syntax file
" Language:	VCU assembly language (based on C syntax)
...
syn keyword     cLabel          dma trace brk select rotate leading0 leading1
syn keyword     cLabel          compare crc crcc sync wait
syn keyword	cConditional	if ifdef include define
syn keyword	cRepeat		trace brk sync wait
syn keyword     cStorageClass   high low unpack 

syn keyword     cConstant       ism
syn keyword     cConstant       from_ism to_ism basic gather scatter gatheri scatteri broadcast initialize

syn match       cConstant       /\<W[0-5]\>/
syn match       cConstant       /\<[ABCD][0-3]\>/

syn match       cType           /\<[u|v|h][0-9]\+\>/
syn match       cType           /\<[u|v|h][0-9]\+\.[l|h]\>/
